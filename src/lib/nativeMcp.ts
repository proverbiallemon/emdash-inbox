...
export function nativeInboxMcp(
	...
	const reads = new Set(["list_threads", "get_thread", "search_messages", "list_drafts", "read_attachment"]);
	for (const tool of listInboxTools()) {
		...
		routes[route] = {
			...
			async handler(ctx) {
				await prepare(ctx);
				try { return await runInboxToolHandler(ctx, tool.name, ctx.input, deliver); }

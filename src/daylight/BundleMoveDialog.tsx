...
export function BundleMoveDialog({row,onClose,onChanged,onRefresh}:{row:ThreadSummary;onClose:()=>void;onChanged:(message:string)=>void;onRefresh:()=>void}) {
 ...
 return <Dialog title="Move conversation" onClose={()=>{if(!lock.current)onClose();}}>
  <div className="dl-bundle-dialog">
   ...
   <label className="dl-checkbox-label"><input name="future" type="checkbox" checked={future} disabled={pending||!sender} onChange={event=>{setFuture(event.target.checked);setReplace(false);}}/>Also move future mail from this exact sender</label>
   ...
   <div className="dl-confirm-actions"><button type="button" className="dl-button dl-subtle" disabled={pending} onClick={onClose}>{partial?'Keep conversation move':'Cancel'}</button><button type="button" className="dl-button dl-primary" disabled={pending||changedSender||(conflict&&!replace)} onClick={()=>void save()}>{pending?'Moving…':partial?'Retry sender rule':'Move conversation'}</button></div>
  </div>
 </Dialog>;
}

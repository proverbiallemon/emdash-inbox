...
export function ThreadCard({ row, busy, selected, onOpen, onPinToggle, onDone, onSnoozeRequest }: Props) {
 ...
 return <article className="dl-thread-card" data-read={row.unreadCount === 0} data-selected={selected}>
  <button type="button" className="dl-thread-open" aria-label={`Open ${subject}`} aria-current={selected ? "true" : undefined} onClick={() => onOpen(row.openMessageId)}>
   ...
   <span className="dl-thread-copy"><strong>{subject}</strong><span>{row.latest.bodyText.replace(/\s+/g, " ").trim() || "No preview available"}</span></span>

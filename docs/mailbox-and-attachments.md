...
After upgrading, refresh MCP consent in the plugin manager by disabling and re-enabling Inbox's MCP tools. EmDash 0.38 binds consent to the exact tool definitions, including schemas. Changed definitions invalidate previous consent even when the switch still appears enabled; clients then see no Inbox tools until consent is renewed. The 0.9 catalog contains 17 tools.
